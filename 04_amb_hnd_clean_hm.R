...
# Read the specific range from the downloaded file
x2024handovers <- read_excel("data/raw2024handovers.xlsx", sheet = 'All handovers', range = "F14:ALR19")


#file should be read in

ambdelay<-x2024handovers %>% 
  t() %>% 
  as.data.frame() %>% 
  select(group=V1, metric=V2, count=V5) %>% 
  fill(group, .direction="down")
...
amball201725 <- rbind(amball201724,amball202425)


saveRDS(amball201725, file="amball201725.rds")
